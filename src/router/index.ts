...
const router = createRouter({
  ...
  routes: [
    ...
    },
    {
      ...
      redirect: (to) =>
        `/game/chinese/${String(to.params.grade ?? '')}/${String(to.params.unit ?? '')}/${String(to.params.level ?? '')}`
    },
    {
      path: '/victory/:grade/:unit/:level',
      redirect: (to) =>
        `/victory/chinese/${String(to.params.grade ?? '')}/${String(to.params.unit ?? '')}/${String(to.params.level ?? '')}`
    },
