...
export function resolveDateRange(preset: DatePreset, customStart = '', customEnd = ''): DateRange {
  const now = Date.now();
...
function buildSessionMap(events: AnalyticsEvent[]): Map<string, AnalyticsEvent[]> {
...
function matchDimensionFilters(event: AnalyticsEvent, filters: DashboardFilters): boolean {
...
function toPageStats(counter: Map<string, { pv: number; uv: Set<string> }>): PageStats[] {
  return Array.from(counter.entries())
    .map(([path, info]) => ({ path, pv: info.pv, uv: info.uv.size }))
    .sort((a, b) => b.pv - a.pv);
}

function countByPath(events: AnalyticsEvent[]): PageStats[] {
  const counter = new Map<string, { pv: number; uv: Set<string> }>();
...
function countEntryOrExitPages(sessionMap: Map<string, AnalyticsEvent[]>, type: 'entry' | 'exit'): PageStats[] {
  const counter = new Map<string, { pv: number; uv: Set<string> }>();
...
function buildFunnel(sessionMap: Map<string, AnalyticsEvent[]>): FunnelStage[] {
  const checks: Array<{ key: string; label: string; test: (event: AnalyticsEvent) => boolean }> = [
    {
      key: 'home',
      label: '首页曝光',
...
function buildLearningRows(events: AnalyticsEvent[]): LearningStatsRow[] {
  type Bucket = {
    subjectId: SubjectId;
    gradeId: GradeId;
    unit: number;
...
function buildDailyTrend(events: AnalyticsEvent[], range: DateRange): TrendPoint[] {
  const dayMap = new Map<string, { devices: Set<string>; starts: number; completes: number }>();

  for (
    let cursor = getDayStart(range.startAt);
    cursor <= range.endAt;
...
export function createDashboardReport(allEvents: AnalyticsEvent[], range: DateRange, filters: DashboardFilters): DashboardReport {
  ...
  const sessionsWithPageView = Array.from(sessionMap.values()).filter((events) =>
    events.some((event) => event.name === 'page_view')
  );

  const bounceSessions = sessionsWithPageView.filter((events) =>
    events.filter((event) => event.name === 'page_view').length <= 1
  ).length;
  ...
  const subjectStats: SubjectStats[] = SUBJECT_META.map((subject) => ({
