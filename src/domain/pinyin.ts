...
function applyTone(syllable: string, tone: number): string {
  ...
  const toneTable: Record<string, string[]> = {
    a: ['a', 'ā', 'á', 'ǎ', 'à'],
    o: ['o', 'ō', 'ó', 'ǒ', 'ò'],
    e: ['e', 'ē', 'é', 'ě', 'è'],
    i: ['i', 'ī', 'í', 'ǐ', 'ì'],
...
function numericToMarked(raw: string): string {
  return raw.replace(/([a-züv:]+)([1-5])/gi, (_, syllable: string, toneNum: string) => {
...
export function displayPinyin(rawPinyin: string, hanzi: string): string {
